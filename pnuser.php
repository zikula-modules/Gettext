 ...
 * @version $Id: pnuser.php 26127 2009-08-07 08:42:12Z drak $
 ...
 */
function gettext_user_main($args)
{
    // security check
...
function gettext_user_extract($args)
{
    // security check
    if (!SecurityUtil::checkPermission('Gettext::', '::', ACCESS_READ)) {
        return LogUtil::registerPermissionError();
    ...
    $pnRender->assign('key', $pid);
    $pnRender->assign('c', $component);
    $pnRender->assign('d', $domain);
    $pnRender->assign('output', $output);
    return $pnRender->fetch('gettext_user_download.htm');
}


function gettext_user_download($args)
{
    // security check
    if (!SecurityUtil::checkPermission('Gettext::', '::', ACCESS_READ)) {
        return LogUtil::registerPermissionError();
...
function gettext_user_compilemo()
{
    ...
    if (!isset($submit)) {
        return $pnRender->fetch('gettext_user_compilemo.htm');
    }
    ...
    $pnRender->assign('key', $pid);
    return $pnRender->fetch('gettext_user_downloadmo.htm');
}

function gettext_user_downloadmo($args)
{
    // security check
    if (!SecurityUtil::checkPermission('Gettext::', '::', ACCESS_READ)) {
        return LogUtil::registerPermissionError();
